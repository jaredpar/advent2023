...
internal sealed partial class Game(int id, List<Set> sets) {
    ...
    internal static Game Parse(string line)
    {
        ...
        var id = int.Parse(line.AsSpan().Slice(start, end - start));
        var matches = GetRegex().Matches(line, end);
        var sets = new List<Set>();
        var open = false;
        (int red, int green, int blue) set = (0, 0, 0);
        foreach (Match match in matches)
        {
            var count = int.Parse(match.Groups[1].Value);
            _ = match.Groups[2].ValueSpan switch
            {
                "red" => set.red += count,
                "green" => set.green += count,
                "blue" => set.blue += count,
                _ => throw new Exception("Invalid cube")
            };

            if (match.Groups[3].Value is ";" or "")
            {
                sets.Add(new Set(set.red, set.green, set.blue));
                set = (0, 0, 0);
            }
            else
            {
    ...
    internal static IEnumerable<Game> ParseAll(string input)
    {
...
internal class Input
{
    ...
    internal static string GetTestInput() => """
